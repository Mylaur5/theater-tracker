...
def process_image(img_element, folder_name):
    ...
    if not file_name:
        print(f"Skipping image with src '{img_src}' due to missing or empty alt attribute.")
        return
    ...
    if os.path.exists(destination_folder):
        ...
        if existing_file_size == new_file_size:
            print(f"Image '{file_name}' already exists and is identical. Skipping download.")
            return
    else:
        img_req = requests.get(img_link, stream=True)
    ...
    print(f"Image '{file_name}' has been successfully downloaded and saved to '{destination_folder}'")
...
def extract():
    ...
    paired_h2_elements = zip(relevant_h2_elements, relevant_h2_elements[1:] + [None])

    # Use map to find the relevant sections and their images
    img_batches = list(map(lambda pair: extract_images(pair[0], pair[1]), paired_h2_elements))
